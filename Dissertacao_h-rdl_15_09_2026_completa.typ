...
#align(right)[
  #block(width: 55%)[
    ...
    Dissertação de Mestrado apresentada ao Programa de Pós-Graduação em Ciência da Computação da Universidade Federal do Pará como requisito parcial para a obtenção do título de Mestre em Ciência da Computação.        *Área de Concentração:* Sistemas de Computação    *Linha de Pesquisa:* Redes de Computadores, Sistemas Distribuídos e Otimização em Telecomunicações    *Orientador:* Prof. Dr. Antônio Jorge Gomes Abelém
  ]
]
...
$ cases(
  x_i + x_j <= 1, & quad forall (a_i, a_j) in cal(E)_t,   sum_(i=1)^m x_i dot "PRB"(a_i) <= "PRB"_(max)^("cell"), & quad forall "Célula " c,   P_(t x)^(min) <= P_(t x)^((0)) + sum_(i=1)^m x_i dot Delta P_(t x)(a_i) <= P_(t x)^(max), & quad forall "Célula " c,   x_i in {0, 1}, & quad forall i in {1, dots, m}.
) $
